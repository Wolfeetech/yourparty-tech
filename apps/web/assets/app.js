...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Use either playToggle OR miniPlayToggle - front-page.php only has miniPlayToggle
  const activePlayButton = playToggle || miniPlayToggle;

  if (audioPlayer && activePlayButton) {
    const playToggleIcon = activePlayButton.querySelector("[aria-hidden]");
    ...
    const updateIcon = () => {
      ...
      if (playToggleIcon) {
        ...
        activePlayButton.textContent = icon;
      }
      if (activePlayButton) {
        activePlayButton.setAttribute("aria-label", label);
      }
    ...
    if (miniPlayToggle) miniPlayToggle.addEventListener("click", togglePlay);
  ...
  if (typeof StationSwitcher !== 'undefined') {
    stationSwitcher = new StationSwitcher({
      ...
      apiBase: REST_BASE,
      streamUrls: {
        1: "https://radio.yourparty.tech/radio.mp3",
        2: "https://radio.yourparty.tech/radio2.mp3"
      }
    });
  }
  ...
  const fetchStatus = async () => {
    try {
      ...
      const response = await fetch(buildEndpoint(`status?station_id=${sid}`), {
